...
function SidebarProvider({
  ...
  // Read the cookie to get the initial state
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'relative flex flex-col flex-1 bg-background max-w-full min-h-svh',
        'peer-data-[variant=inset]:min-h-[calc(100svh-(--spacing(4)))]  md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-0',
        className,
      )}
      {...props}
    />
...
function SidebarGroup({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar="group"
      className={cn('relative flex flex-col w-full min-w-0', className)}
      {...props}
    />
  )
}
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'flex items-center rounded-md outline-hidden ring-sidebar-ring focus-visible:ring-2 h-8 [&>svg]:size-4 font-medium text-sidebar-foreground/70 text-xs transition-[margin,opacity] duration-200 ease-linear px shrink-0 [&>svg]:shrink-0',
